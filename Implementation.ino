/*  Project: Burglar Alarm System
    Group 24
    Description:
    implements a facial-recognition-based alarm system
    with sensor monitoring (motion, door), PIN authentication, and control
    over output devices such as LEDs, buzzer, and a solenoid lock.
  
*/

// Webcam class for handling face authentication
class Webcam {
  private:
    bool enabled = false;
    int faceAttempts = 0;
    ...
    bool awaitFaceRecognition() {
      ...
      if (Serial.available() > 0) {
        ...
        if (msg == "FACE_OK") {
          ...
          faceAttempts++; // used to track the number of failed attempts of the facial recognition
          enabled = false;
          return false;
        } 
        else {
...
// Sensor class for digital input monitoring
class Sensor {
...
// Base class for alarm output devices
class AlarmOutput {
...
// Central class that controls the entire alarm session
class MainSession {
...
void loop() {
  ...
  bool selectionMade = false;
  // Wait for initial user input via serial (change PIN or arm system)
  while (!selectionMade) {
      // Check for incoming serial data
      if (Serial.available()) {
          String input = Serial.readStringUntil('\n');
  ...
  // Main loop when alarm is active
