function [verified] = streamingFaceRecognition(nOfEach,pauseval, option)
...
if numel(imgSet) < 2
    verified = streamingFaceRecognition(8,0.1, 1);
end
if refreshOption ~= 3
	queryForNames;
end
...
verified = false;
faceDetected = false;
a = annotation('textbox', [0.15, 0.85, 0.3, 0.1]);

%%% Okay, so now we should have a recognizer in place!!!
f = figure(fdrFig);
while double(get(fdrFig,'currentCharacter')) ~= 27 && ~isDone && (score < 20)
	bestGuess = '?';
	RGBFrame = snapshot(vidObj);
	...
	%text(f, 'Name', "Face detected: "+ faceDetected+ " User: "+ bestGuess +" Score:" +score+ "/20");
    delete(a);
    a = annotation('textbox', [0.15, 0.85, 0.3, 0.1], ...
        'String', sprintf("Face detected: "+ faceDetected+ " \nUser: "+ bestGuess +" \nScore:" +score+ "/20"), ...
        'FontSize', 16);
end %while
...
% Clear the webcam object if it exists
if ~isempty(vidObj)
    clear vidObj
    vidObj = [];
end
...
function deleteUserFolder()
    ...
    userFolders = dir(rootDir);
    userFolders = userFolders([userFolders.isdir]); % Only keep directories
    userFolders = userFolders(~ismember({userFolders.name}, {'.', '..'})); % Remove '.' and '..'
    folderNames = {userFolders.name};
    fig = uifigure('Name', 'Delete User Folder', 'Position', [500 500 300 150]);
    dd = uidropdown(fig, ...
        'Position', [75 75 150 22], ...
        'Items', folderNames, ...
        'Placeholder', 'Select User Folder');
    btn = uibutton(fig, 'Position', [100 30 100 30], 'Text', 'Delete Folder');
    btn.ButtonPushedFcn = @(btn, event) deleteFolder(dd, rootDir, fig);
end

function deleteFolder(dd, rootDir, fig)
    close(fig);
    
    selectedFolder = dd.Value;
    if isempty(selectedFolder)
        %uialert(gcf, 'No folder selected!', 'Error');
        return;
    end
    folderPath = fullfile(rootDir, selectedFolder);
    choice = questdlg(['Are you sure you want to delete the folder: ', selectedFolder], ...
        'Confirm Deletion', 'Yes', 'No', 'No');
    if strcmp(choice, 'Yes')
        try
            rmdir(folderPath, 's');
            %uialert(gcf, ['Folder "', selectedFolder, '" deleted successfully.'], 'Success');
        catch
            %uialert(gcf, 'Error deleting folder. Please check permissions.', 'Error');
        end
    end
end
