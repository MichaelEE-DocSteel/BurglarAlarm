function streamingFaceRecognition(nOfEach,pauseval, option)
...
userTable = readtable('users.csv')
if isempty(userTable)
    userTable = table("Person", 1);
    writetable(userTable, "users.csv");
    writetable(table("Person", 1), "users.csv");
end
...
if refreshOption == 1
	rmdir(targetDirectory,'s');  %% Remove all
	mkdir(targetDirectory)
...
elseif refreshOption == 2
    ...
    userTable = [userTable; {"Person", personNumber + 1}]
    writetable(userTable, "users.csv");
...
if numel(imgSet) < 2
    close all force ;
	disp('streamingFaceRecognition: You must capture at least two individuals for this to work!');
    input();
end
if refreshOption ~= 3
	queryForNames;
